...
namespace vpt
{
  ...
  struct EuclideanMetricPbc
  {
    ...
    EuclideanMetricPbc(Vector dims, std::vector<bool> p) : dimensions(dims), periodic(p) {}
  ...
  {
  public:
    DimensionMismatch(int expected, int got) : std::runtime_error("Item dimension doesn't match: expected " + std::to_string(expected) + ", got " + std::to_string(got)) {}
  };
  ...
  private:
    struct Node
    {
      ...
      Node(int item, double threshold = 0., int left = Leaf, int right = Leaf) : item(item), threshold(threshold), left(left), right(right) {}
  ...
  template <typename InputIterator>
  VpTree::VpTree(InputIterator start, InputIterator end, Metric metric) : getDistance(metric), items_(makeItems(start, end)), nodes_(), rng_()
  {
  ...
  template <typename Container>
  VpTree::VpTree(const Container &container, Metric metric) : VpTree(container.begin(), container.end(), metric) {}

  VpTree::VpTree(std::initializer_list<Vector> list, Metric metric) : VpTree(list.begin(), list.end(), metric) {}
...
#pragma omp parallel for schedule(dynamic)
  ...
  Searcher::Searcher(const VpTree *tree, const Vector &target, int neighborsCount) : tree_(tree), target_(target), neighborsCount_(neighborsCount), tau_(std::numeric_limits<double>::max()), heap_() {}
  Searcher::Searcher(const VpTree *tree, const Vector &target, double radius) : tree_(tree), target_(target), radius_(radius), tau_(std::numeric_limits<double>::max()), heap_(), neighborsCount_(0)
  {
    tau_ = radius;
  }
